...
def load_rows(root):
    ...
    for path in sorted(root.glob('**/*-result.json')):
        ...
        row.update(population=population,idle_calibration_eligible=policies.get(population,{}).get('idle_calibration_eligible',True))
...
def report(rows):
    ...
    for r in rows:
        if r['environment_eligible']:
            groups[(r['population'],r['profile'],r['fixture'],r['stage'],r['cache'],r['slots'],r['mtp'],r['chunk'],r['context'])].append(r)
    summary=[]
    for key, values in sorted(groups.items()):
        fields=('population','profile','fixture','stage','cache','slots','mtp','chunk','context')
