...
def probe(path,pid=None):
    return json.loads(subprocess.check_output([str(path)]+([str(pid)] if pid else []),text=True,timeout=5))
...
def settle(probe_path,seconds,need_gb,timeout=900,lock_required=True):
    start=time.monotonic();stable=None;observations=[];previous=None;last=start-30
    while True:
        now=time.monotonic();state=probe(probe_path);vm=vm_snapshot();jobs=s.competing_jobs()
        lockfree=True
        if lock_required:
        ...
        if stable is not None and now-stable>=seconds:return observations
        if now-start>=timeout:raise TimeoutError('readiness timeout; no model launched' if lock_required else 'thermal readiness timeout')
        if now-last>=30:
            emit({'phase':'readiness','wait_s':round(now-start),'stable_s':round(now-stable) if stable else 0,'need_gb':need_gb,'free_gb':round(vm['reclaimable_bytes']/1e9,1),'state':state});last=now
        time.sleep(2)
...
def request(child,port,entry,cap,profile,p,cell,stage,round_number):
    ...
    result.update(profile=profile['id'],fixture=entry['id'],kind=entry['kind'],stage=stage,round=round_number,before=before,after=after,own_before=own_before,own_after=own_after,samples=monitor.samples,exclusions=sorted(set(reasons)),environment_eligible=not reasons)
...
def run_session(p,profile,round_number,out,phase):
    ...
    try:
        ...
        if plan['device_available_gb']<plan['target_gb']+p['minimum_spare_gb']:raise RuntimeError('actual auto plan leaves insufficient preflight spare memory')
        ...
        with (session/'server.stdout').open('wb') as stdout,(session/'server.stderr').open('wb') as stderr:
            ...
            for index,rid in enumerate(ids):
                ...
                settle(p['probe'],p['between_requests_nominal_seconds'],0,p['readiness_timeout_seconds'],lock_required=False)
                if phase=='decode':
