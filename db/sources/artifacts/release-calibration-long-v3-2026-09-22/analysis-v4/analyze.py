...
def analyse(root):
 ...
 for f in sorted(root.glob('**/*-result.json')):
  ...
  if policy.get('idle_calibration_eligible') is not True:observational+=1;continue
  r=adjudicate(r,str(f.relative_to(root)),policies)
  r.update(request_history(f,r))
 ...
 for item in result['prefill']:
  rows=prefill[(item['profile'],item['fixture'],item['stage'])]
  strata=[]
  for history in sorted({r['server_history'] for r in rows}):
   subset=[r for r in rows if r['server_history']==history and r.get('environment_eligible') and (item['stage']!='prefill_miss' or r['metrics']['stats']['reusedPrefixTokens']==0)]
   strata.append({'server_history':history,'eligible':len(subset),'prefill_seconds':[r['metrics']['stats']['prefillSeconds'] for r in subset],'read_batches':[r['metrics']['stats']['prefillPasses'] for r in subset]})
  item['history_strata']=strata
  item['qualification_scope']='At least three eligible observations of this prompt/plan across the frozen request order. History strata above remain separate; this is not a history-independent ETA anchor.'
 return result
